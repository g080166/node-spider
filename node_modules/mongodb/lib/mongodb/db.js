 ...
 * @ignore
 */
var QueryCommand = require('./commands/query_command').QueryCommand, DbCommand = require('./commands/db_command').DbCommand, MongoReply = require('./responses/mongo_reply').MongoReply, Admin = require('./admin').Admin, Collection = require('./collection').Collection, Server = require('./connection/server').Server, ReplSet = require('./connection/repl_set/repl_set').ReplSet, ReadPreference = require('./connection/read_preference').ReadPreference, Mongos = require('./connection/mongos').Mongos, Cursor = require('./cursor').Cursor, EventEmitter = require('events').EventEmitter, InsertCommand = require('./commands/insert_command').InsertCommand, f = require('util').format, inherits = require('util').inherits, crypto = require('crypto'), timers = require('timers'), utils = require('./utils')

// Authentication methods
, mongodb_cr_authenticate = require('./auth/mongodb_cr.js').authenticate, mongodb_gssapi_authenticate = require('./auth/mongodb_gssapi.js').authenticate, mongodb_sspi_authenticate = require('./auth/mongodb_sspi.js').authenticate, mongodb_plain_authenticate = require('./auth/mongodb_plain.js').authenticate, mongodb_x509_authenticate = require('./auth/mongodb_x509.js').authenticate, mongodb_scram_authenticate = require('./auth/mongodb_scram.js').authenticate;
...
function Db(databaseName, serverConfig, options) {
	if (!(this instanceof Db))
		return new Db(databaseName, serverConfig, options);
	...
	var overrideUsedFlag = this.options['override_used_flag'] == null ? false
			: this.options['override_used_flag'];

	// Verify that nobody is using this config
	if (!overrideUsedFlag && this.serverConfig != null
			&& typeof this.serverConfig == 'object'
			&& this.serverConfig._isUsed && this.serverConfig._isUsed()) {
		throw new Error(
	...
	this.slaveOk = this.options['slave_ok'] == null ? false
			: this.options['slave_ok'];
	this.slaveOk = this.options['slaveOk'] == null ? this.slaveOk
			: this.options['slaveOk'];

	// Number of operations to buffer before failure
	this.bufferMaxEntries = typeof this.options['bufferMaxEntries'] == 'number' ? this.options['bufferMaxEntries']
			: -1;
	...
	try {
		...
		// The bson lib
		var bsonLib = this.bsonLib = this.options.native_parser ? require('bson').BSONNative
				: require('bson').BSONPure;
		bsonLib = require('bson').BSONPure;
		// Fetch the serializer object
		var BSON = bsonLib.BSON;

		// Create a new instance
		this.bson = new BSON([ bsonLib.Long, bsonLib.ObjectID, bsonLib.Binary,
				bsonLib.Code, bsonLib.DBRef, bsonLib.Symbol, bsonLib.Double,
				bsonLib.Timestamp, bsonLib.MaxKey, bsonLib.MinKey ]);
		this.bson.promoteLongs = this.options.promoteLongs == null ? true
		...
		this.bson_deserializer.promoteLongs = this.options.promoteLongs == null ? true
				: this.options.promoteLongs;
	} catch (err) {
	...
	this.pkFactory = this.options.pkFactory == null ? bsonLib.ObjectID
			: this.options.pkFactory;
	this.forceServerObjectId = this.options.forceServerObjectId != null ? this.options.forceServerObjectId
			: false;

	// Added safe
	this.safe = this.options.safe == null ? false : this.options.safe;

	// If we have not specified a "safe mode" we just print a warning to the
	// console
	if (this.options.safe == null && this.options.w == null
			&& this.options.j == null && this.options.journal == null
			&& this.options.fsync == null) {
		console
				.log("========================================================================================");
		console
				.log("=  Please ensure that you set the default write concern for the database by setting    =");
		console
				.log("=   one of the options                                                                 =");
		console
				.log("=                                                                                      =");
		console
				.log("=     w: (value of > -1 or the string 'majority'), where < 1 means                     =");
		console
				.log("=        no write acknowledgement                                                       =");
		console
				.log("=     journal: true/false, wait for flush to journal before acknowledgement             =");
		console
				.log("=     fsync: true/false, wait for flush to file system before acknowledgement           =");
		console
				.log("=                                                                                      =");
		console
				.log("=  For backward compatibility safe is still supported and                              =");
		console
				.log("=   allows values of [true | false | {j:true} | {w:n, wtimeout:n} | {fsync:true}]      =");
		console
				.log("=   the default value is false which means the driver receives does not                =");
		console
				.log("=   return the information of the success/error of the insert/update/remove            =");
		console
				.log("=                                                                                      =");
		console
				.log("=   ex: new Db(new Server('localhost', 27017), {safe:false})                           =");
		console
				.log("=                                                                                      =");
		console
				.log("=   http://www.mongodb.org/display/DOCS/getLastError+Command                           =");
		console
				.log("=                                                                                      =");
		console
				.log("=  The default of no acknowledgement will change in the very near future                =");
		console
				.log("=                                                                                      =");
		console
				.log("=  This message will disappear when the default safe is set on the driver Db           =");
		console
				.log("========================================================================================");
	}
	...
	// Command queue, keeps a list of incoming commands that need to be executed
	...
	this.logger = this.options.logger != null
			...
			&& (typeof this.options.logger.log == 'function') ? this.options.logger
			: {
	...
	if (this.serverConfig.strategyInstance)
		this.serverConfig.strategyInstance.logger = this.logger;
	...
	this.serializeFunctions = this.options.serializeFunctions != null ? this.options.serializeFunctions
			: false;
	...
	// Record query stats
	this.recordQueryStats = this.options.recordQueryStats != null ? this.options.recordQueryStats
			: false;

	// If we have server stats let's make sure the driver objects have it
	// enabled
	...
	// Retry information
	this.retryMiliSeconds = this.options.retryMiliSeconds != null ? this.options.retryMiliSeconds
			: 1000;
	this.numberOfRetries = this.options.numberOfRetries != null ? this.options.numberOfRetries
			: 60;

	// Set default read preference if any
	this.readPreference = this.options.readPreference;

	// Set slaveOk if we have specified a secondary or secondary preferred
	// readPreference
	...
	if (this.serverConfig.options.readPreference != null) {
		this.serverConfig
				.setReadPreference(this.serverConfig.options.readPreference);
...
function validateDatabaseName(databaseName) {
	...
	for (var i = 0; i < invalidChars.length; i++) {
		if (databaseName.indexOf(invalidChars[i]) != -1)
			throw new Error("database names cannot contain the character '"
					+ invalidChars[i] + "'");
	}
}
...
Db.prototype.open = function(callback) {
	...
	if (self.serverConfig instanceof Server
			|| self.serverConfig instanceof ReplSet
		...
		self.serverConfig.connect(self, connect_options, function(err, result) {
			...
			// If we have queued up commands execute a command to trigger
			// replays
			if (self.commands.length > 0)
				_execute_queued_command(self);
			// Callback
		...
		try {
			callback(
					Error("Server parameter must be of type Server, ReplSet or Mongos"),
					null);
		} catch (err) {
			self.close();
			throw err;
...
Db.prototype.listCollections = function(name, options, callback) {
	...
	var fallbackListCollections = function() {
		...
		if (typeof name == 'string')
			filter.name = f("%s.%s", self.databaseName, name);
		// Get the system namespace collection as a cursor
		var cursor = self.collection(DbCommand.SYSTEM_NAMESPACE_COLLECTION)
				.find(filter);
		// Get all documents
	...
	if (filter)
		listCollectionsCommand.filter = filter;
	// Set the name variable for the filter
	if (typeof name == 'string')
		filter.name = name;
	...
	self.command(listCollectionsCommand, function(err, result) {
		if (err)
			return fallbackListCollections();
...
Db.prototype.collection = function(collectionName, options, callback) {
	...
	if (options == null || !options.strict) {
		try {
			var collection = new Collection(self, collectionName,
					self.pkFactory, options);
			if (callback)
	...
	if (typeof callback != 'function') {
		throw utils
				.toError(f(
						"A callback is required in strict mode. While getting collection %s.",
						collectionName));
	}
	...
	self.listCollections(collectionName, function(err, collections) {
		...
		if (collections.length == 0)
			return callback(utils.toError(f(
					"Collection %s does not exist. Currently in strict mode.",
					collectionName)), null);

		try {
			return callback(null, new Collection(self, collectionName,
					self.pkFactory, options));
		} catch (err) {
			return callback(err, null);
		}
...
Db.prototype.collections = function(callback) {
	...
	self.collectionNames(function(err, documents) {
		...
		documents.forEach(function(document) {
			collections.push(new Collection(self, document.name.replace(
					self.databaseName + ".", ''), self.pkFactory));
		});
...
Db.prototype.eval = function(code, parameters, options, callback) {
	...
	this.command(cmd, options, function(err, result) {
		...
		if (result && result.ok == 1)
			return callback(null, result.retval);
		if (result)
			return callback(new Error("eval failed: " + result.errmsg), null);
		callback(err, result);
	});
};
...
Db.prototype.authenticate = function(username, password, options, callback) {
	...
	if (!options.authMechanism) {
		...
		return callback(new Error(
				"only GSSAPI, PLAIN, MONGODB-X509, SCRAM-SHA-1 or MONGODB-CR is supported by authMechanism"));
	}

	// the default db to authenticate against is 'this'
	// if authententicate is called from a retry context, it may be another one,
	// like admin
	...
	if (options.authMechanism == 'MONGODB-CR') {
		mongodb_cr_authenticate(self, username, password, authdb, options,
				_callback);
		...
		mongodb_scram_authenticate(self, username, password, authdb, options,
				_callback);
		...
		if (servers.length > 0 && servers[0].isMasterDoc
				&& servers[0].isMasterDoc.maxWireVersion >= 3) {
			mongodb_scram_authenticate(self, username, password, authdb,
					options, _callback);
		} else {
			mongodb_cr_authenticate(self, username, password, authdb, options,
					_callback);
		...
		if (hasKerberos == false) {
			console
					.log("========================================================================================");
			console
					.log("=  Please make sure that you install the Kerberos library to use GSSAPI                =");
			console
					.log("=                                                                                      =");
			console
					.log("=  npm install -g kerberos                                                             =");
			console
					.log("=                                                                                      =");
			console
					.log("=  The Kerberos package is not installed by default for simplicities sake              =");
			console
					.log("=  and needs to be global install                                                      =");
			console
					.log("========================================================================================");
		...
		if (process.platform == 'win32') {
			mongodb_sspi_authenticate(self, username, password, authdb,
					options, _callback);
		} else {
			// We have the kerberos library, execute auth process
			mongodb_gssapi_authenticate(self, username, password, authdb,
					options, _callback);
		}
	}
};
...
Db.prototype.addUser = function(username, password, options, callback) {
	...
	if (connection != null && connection.serverCapabilities != null
			&& connection.serverCapabilities.hasAuthCommands) {
		return _executeAuthCreateUserCommand(this, username, password, options,
				callback);
	}
 ...
 * @ignore
 */
var _executeAuthCreateUserCommand = function(self, username, password, options,
		callback) {
	...
	if (options.digestPassword != null) {
		throw utils
				.toError("The digestPassword option is not supported via add_user. Please use db.command('createUser', ...) instead for this option.");
	}

	// Get additional values
	var customData = options.customData != null ? options.customData : {};
	var roles = Array.isArray(options.roles) ? options.roles : [];
	var maxTimeMS = typeof options.maxTimeMS == 'number' ? options.maxTimeMS
			: null;

	// If not roles defined print deprecated message
	if (roles.length == 0) {
		console
				.log("Creating a user without roles is deprecated in MongoDB >= 2.6");
	}
	...
	self.command(command, commandOptions, function(err, result) {
		if (err)
			return callback(err, null);
		callback(!result.ok ? utils.toError("Failed to add user " + username)
				: null, result.ok ? [ {
			user : username,
			pwd : ''
		} ] : null);
...
Db.prototype.removeUser = function(username, options, callback) {
	...
	var safe = self.safe != null && self.safe == false ? {
		w : 1
	} : self.safe;
	...
	collection.findOne({
		...
		if (user != null) {
			...
			collection.remove({
				user : username
			}, commandOptions, function(err, result) {
				callback(err, true);
			});
		} else {
			callback(err, false);
...
Db.prototype.createCollection = function(collectionName, options, callback) {
	...
	this.listCollections(collectionName, function(err, collections) {
		...
		if (collections.length > 0 && options.strict) {
			return callback(utils.toError(f(
					"Collection %s already exists. Currently in strict mode.",
					collectionName)), null);
		...
		self.command(cmd, options, function(err, result) {
			if (err && err.code && err.code != 48 && options && options.strict)
				return callback(err, null);
			try {
				callback(null, new Collection(self, collectionName,
						self.pkFactory, options));
			} catch (err) {
...
Db.prototype.command = function(selector, options, callback) {
	...
	var ignoreCommandFilter = options.ignoreCommandFilter ? options.ignoreCommandFilter
			: false;

	// Get read preference if we set one
	var readPreference = _getReadConcern(this, options);

	// Ensure only commands who support read Prefrences are exeuted otherwise
	// override and use Primary
	...
	if (typeof options['checkKeys'] == 'boolean')
		command_options['checkKeys'] = options['checkKeys'];
	command_options['checkKeys'] = typeof options['checkKeys'] == 'boolean' ? options['checkKeys']
			: false;
	if (typeof options['serializeFunctions'] == 'boolean')
		command_options['serializeFunctions'] = options['serializeFunctions'];
	if (options['dbName'])
		command_options['dbName'] = options['dbName'];

	// If we have a write command, remove readPreference as an option
	if ((options.writeCommand || selector['findAndModify']
	...
	if (options.w || options.wtimeout || options.j || options.fsync
		...
		if (options.safe)
			selector.writeConcern.w = 1;
		if (options.w)
			selector.writeConcern.w = options.w;
		if (options.wtimeout)
			selector.writeConcern.wtimeout = options.wtimeout;
		if (options.j)
			selector.writeConcern.j = options.j;
		if (options.fsync)
			selector.writeConcern.fsync = options.fsync;
	}
	...
	execDb = typeof options.authdb == 'string' ? this.db(options.authdb)
			: execDb;

	// Execute a query command
	this._executeQueryCommand(DbCommand.createDbSlaveOkCommand(execDb,
			selector, command_options), options, function(err, results,
			connection) {
		if (options.returnConnection) {
			if (err)
				return callback(err, null, connection);
			if (results == null || results.documents == null)
				return callback(new Error("command failed to return result"));
...
Db.prototype.executeDbCommand = function(command_hash, options, callback) {
	...
	this._executeQueryCommand(DbCommand.createDbSlaveOkCommand(this,
			command_hash, options), options, function(err, result) {
		if (callback)
			callback(err, result);
	});
};
...
Db.prototype.executeDbAdminCommand = function(command_hash, options, callback) {
	...
	this._executeQueryCommand(DbCommand
			.createAdminDbCommand(this, command_hash), options, function(err,
			result) {
		if (callback)
			callback(err, result);
	});
...
Db.prototype.createIndex = function(collectionName, fieldOrSpec, options,
	...
	createIndexUsingCreateIndexes(
			...
			function(err, result) {
				...
				// Create command
				var command = createCreateIndexCommand(self, collectionName,
						fieldOrSpec, options);
				// Default command options
				var commandOptions = {};

				// If we have error conditions set handle them
				if (_hasWriteConcern(writeConcern)
							...
							._executeInsertCommand(
									...
									function(err, result) {
										...
										if (result == null
											...
											return callback(new Error(
													"command failed to return result"));
										...
										if (result[0].err) {
											...
											callback(null,
													command.documents[0].name);
										}
									});
				} else {
					// Execute insert command
					var result = self._executeInsertCommand(command,
...
var createCreateIndexCommand = function(db, collectionName, fieldOrSpec,
		options) {
	...
	var indexName = typeof options.name == 'string' ? options.name
	...
	var finalUnique = options == null || 'object' === typeof options ? false
			: options;
...
var createIndexUsingCreateIndexes = function(self, collectionName, fieldOrSpec,
		options, callback) {
	// Build the index
	var indexParameters = utils.parseIndexOptions(fieldOrSpec);
	// Generate the index name
...
Db.prototype.ensureIndex = function(collectionName, fieldOrSpec, options,
	...
	// Create command
	var command = createCreateIndexCommand(this, collectionName, fieldOrSpec,
			options);
	var index_name = command.documents[0].name;

	// Check if the index allready exists
	this.indexInformation(collectionName, writeConcern, function(err,
			indexInformation) {
		if (err != null)
			return callback(err, null);
...
Db.prototype.indexInformation = function(name, options, callback) {
	...
	if (name == null)
		throw new Error("A collection name must be provided as first argument");
	...
	var fallbackListIndexes = function() {
		...
		var selector = name != null ? {
			ns : (self.databaseName + "." + name)
		} : {};
		...
		collection.find(selector).setReadPreference(readPreference).toArray(
	...
	self.command({
		...
		if (err)
			return fallbackListIndexes();
		// if full defined just return all the indexes directly
		if (full)
			return callback(null, result.indexes);
		// Return all the indexes
...
var __executeQueryCommand = function(self, db_command, options, callback) {
	// Options unpacking
	var readPreference = options.readPreference != null ? options.readPreference
			: 'primary';
	var onAll = options['onAll'] != null ? options['onAll'] : false;
	var specifiedConnection = options['connection'] != null ? options['connection']
			: null;
	var raw = typeof options.raw == 'boolean' ? options.raw : false;

	// Correct readPreference preference to default primary if set to false,
	// null or primary
	if (!(typeof readPreference == 'object')
			&& readPreference._type == 'ReadPreference') {
		readPreference = (readPreference == null || readPreference == 'primary' || readPreference == false) ? ReadPreference.PRIMARY
				: readPreference;
		if (!ReadPreference.isValid(readPreference))
			return callback(new Error("Illegal readPreference mode specified, "
					+ JSON.stringify(readPreference)));
	} else if (typeof readPreference == 'object'
			&& readPreference._type == 'ReadPreference') {
	...
	// If we have a read preference set and we are a mongos pass the read
	// preference on to the mongos instance,
	...
	if (typeof callback === 'function' && !onAll) {
		...
		var connection = specifiedConnection != null ? specifiedConnection
				: null;

		if (connection instanceof Error)
			return callback(connection, null);

		// Fetch either a reader or writer dependent on the specified
		// readPreference option if no connection
		// was passed in
		if (connection == null) {
		...
		self.serverConfig._registerHandler(db_command, raw, connection,
				exhaust, callback);
		...
		connection.write(db_command, function(err) {
			if (err != null) {
				...
				if (Array.isArray(db_command))
					self.serverConfig._callHandler(
							db_command[0].getRequestId(), null, err);
				else
					self.serverConfig._callHandler(db_command.getRequestId(),
							null, err);
			}
		});
			...
			connection.write(db_command,
					function(err) {
						...
						if (err != null) {
							...
							self.serverConfig._removeHandler(db_command
									.getRequestId());
						}
		...
		// Fetch either a reader or writer dependent on the specified read
		// option
		var connection = self.serverConfig.checkoutReader(readPreference);
		// Override connection if needed
		connection = specifiedConnection != null ? specifiedConnection
				: connection;
		// Ensure we have a valid connection
...
Db.prototype._executeQueryCommand = function(db_command, options, callback) {
	...
	if (!connection && !config.canRead(readPreference) && !config.canWrite()
		...
		if (!self.serverConfig._commandsStore
				.validateBufferLimit(self.bufferMaxEntries)) {
			self.close();
		}
	} else if (!connection && !config.canRead(readPreference)
			&& !config.canWrite() && !config.isAutoReconnect()) {
		return callback(new Error("no open connections"), null);
	} else {
		if (typeof callback == 'function') {
			__executeQueryCommand(self, db_command, options, function(err,
					result, conn) {
...
var __executeInsertCommand = function(self, db_command, options, callback) {
	...
	if (connection && !connection.isCompatible()) {
		return callback(utils
				.toError("driver is incompatible with this server version"),
				null);
	}
	...
	if (typeof callback === 'function') {
		...
		if (errorOptions.w > 0 || errorOptions.w == 'majority'
				|| errorOptions.j || errorOptions.journal || errorOptions.fsync) {
			// db command is now an array of commands (original command +
			// lastError)
			db_command = [ db_command,
					DbCommand.createGetLastErrorCommand(errorOptions, self) ];
			// Register the handler in the data structure
			self.serverConfig._registerHandler(db_command[1], false,
					connection, callback);
		}
	}
	...
	if (connection == null)
		return null;
	if (connection instanceof Error && typeof callback == 'function')
		return callback(connection, null);
	if (connection instanceof Error)
		return null;
	if (connection == null && typeof callback == 'function')
		return callback(new Error("no primary server found"), null);

	// Write the message out
	connection.write(db_command, function(err) {
		// Return the callback if it's not a safe operation and the callback is
		// defined
		if (typeof callback === 'function' && (safe == null || safe == false)) {
			...
			// Call the handler with an error
			self.serverConfig._callHandler(db_command[1].getRequestId(), null,
					err);
...
Db.connect = function(url, options, callback) {
	...
	if (url.indexOf("safe") == -1 && url.indexOf("w") == -1
...
var _hasWriteConcern = function(errorOptions) {
	return errorOptions == true || errorOptions.w > 0
			|| errorOptions.w == 'majority' || errorOptions.j == true
			|| errorOptions.journal == true || errorOptions.fsync == true
};
...
var _getWriteConcern = function(self, options) {
	...
	} else if (self.safe.w != null || typeof self.safe.j == 'boolean'
